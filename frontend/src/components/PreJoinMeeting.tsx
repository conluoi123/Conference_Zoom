...
interface PreJoinMeetingProps {
  ...
  onCancel: () => void;
  initialRoomId?: string; // có ? là kiểu optional này ko bắt buộc 
  initialPeerId?: string;
  initialDisplayName?: string;
}

export default function PreJoinMeeting({ onJoinMeeting, onCancel, initialRoomId = "", initialPeerId = "", initialDisplayName = "Guest" }: PreJoinMeetingProps) {
      ...
      <div className="flex-1 relative flex items-center justify-center bg-black rounded-2xl ml-1">
        ...
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex gap-4 justify-around items-center">
          ...
          <button
            ...
            className={`w-14 h-14 rounded-full flex items-center justify-center transition-all ${isCameraOn
              ? 'bg-gray-700 hover:bg-gray-600'
              : 'bg-red-500 hover:bg-red-600'
              }`}
          >
