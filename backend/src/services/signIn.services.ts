...
async function outlookLogInCallback(req: Request, res: Response){
    ...
    await new Promise(resolve => (req as RequestWithUser).session.destroy(resolve));
    ...
    const hashRefToken = crypto.createHash("sha256").update(refToken).digest("hex");
...
export {supportSendOtp, generateOtp, supportVerifyOtp, outlookLogIn, outlookLogInCallback, verifyGoogleToken}